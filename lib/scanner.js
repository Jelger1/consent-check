...
export async function scanUrl(url, instellingen, log = () => {}) {
  ...
  try {
    ...
    // Een site die de scanner weert (botdetectie, 403) levert een bijna lege
    // pagina op. "Geen banner" zou dan een verkeerde conclusie zijn; daarom
    // een waarschuwing die boven het rapport komt te staan.
    const status = scan.meting1.navigatie_status;
    if (status !== null && status >= 400) {
      const melding = `Let op: de pagina gaf status ${status}. De site heeft de scanner waarschijnlijk geblokkeerd (botdetectie); wat hieronder staat is dan de blokkadepagina, niet de echte site.`;
      scan.waarschuwingen.push(melding);
      log(`! ${melding}`);
    } else if (scan.meting1.requests.length < 5 && (scan.html?.lengte ?? 0) < 20_000) {
      const melding = `Let op: de pagina laadde maar ${scan.meting1.requests.length} request(s). Mogelijk is de site niet echt geladen of blokkeert hij de scanner; controleer de site handmatig.`;
      scan.waarschuwingen.push(melding);
      log(`! ${melding}`);
    }
