...
function cookiesVoorConsent(cookies) {
  const relevant = cookies.filter((cookie) => cookie.classificatie !== 'cmp' && cookie.classificatie !== 'functioneel');
  return {
    ...
    uitgesloten: cookies
      ...
      .map((cookie) => ({ naam: cookie.naam, domein: cookie.domein, reden: cookie.classificatie })),
  };
}
