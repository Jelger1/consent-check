...
export async function snapshotDom(page, { cmpIds = [] } = {}) {
  ...
  // Navigeert de pagina precies tijdens de snapshot (een late redirect, of een
  // CMP die herlaadt), dan gooit Playwright "execution context destroyed".
  // Eén keer opnieuw proberen na de navigatie levert alsnog een echte meting op.
  for (let poging = 0; poging < 2 && !snapshot; poging += 1) {
    try {
      snapshot = await page.evaluate(domSnapshot, { cmpIds });
    } catch (error) {
      laatsteFout = error;
      const melding = error.message.split('\n')[0];
      if (!/execution context|destroyed|navigating|frame was detached/i.test(melding) || poging === 1) break;
      await page.waitForLoadState('domcontentloaded', { timeout: 10000 }).catch(() => {});
    }
  }

  if (!snapshot) {
    const error = laatsteFout || new Error('onbekend');
    return {
