...
export function consentAlGegeven(dom) {
  ...
  if (dom?.usercentrics?.alles_geaccepteerd === true) {
    return { gegeven: true, cmp: 'Usercentrics', detail: 'UC_UI.areAllConsentsAccepted() is al true' };
  }
  return { gegeven: false, cmp: null, detail: null };
}

/** De CMP waar regel 5 over gaat: eerst die waar consent doorheen ging, anders de eerste actieve, anders de eerste herkende. */
export function kiesPrimaireCmp(cmps, consent) {
  const geslaagd = new Set((consent?.geprobeerd || []).filter((p) => p.gelukt && p.id).map((p) => p.id));
  const kandidaat = cmps.find((cmp) => geslaagd.has(cmp.id)) || cmps.find((cmp) => cmp.actief) || cmps[0] || null;
  return kandidaat ? CMPS.find((cmp) => cmp.id === kandidaat.id) || null : null;
}
