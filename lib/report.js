...
import { beschrijfCookieScript } from './cmp/cookiescript.js';
import { hostVan } from './domain.js';
import { seconden, slug, tijdstempel } from './util.js';

export const TOOL = { naam: 'consent-check', versie: '1.0.0', cmp_ondersteuning: ['CookieScript'] };
...
export function bouwRapport(scan, instellingen) {
  ...
  if (meting1) {
    try {
      cookiescript = beschrijfCookieScript({
        html,
        dom: meting1.dom,
  ...
  return {
    ...
    cmp_info: {
      ...
      load_position: cookiescript?.load_position ?? null,
      attributes: cookiescript?.attributes ?? [],
      loaded_via_gtm: cookiescript?.loaded_via_gtm ?? null,
      synchroon: cookiescript?.synchroon ?? null,
      cookiescript,
      cmps: scan.cmps,
      google_consent_mode: {
...
export function samenvatting(rapport) {
  ...
  regels.push(`  CookieScript: ${cmp.cookiescript?.detected ? `${cmp.load_position ?? '?'}, ${cmp.attributes.join(' ') || 'geen attributen'}, geladen via ${cmp.cookiescript.geladen_via}` : 'niet aangetroffen'}`);
  ...
  regels.push(`  4. Ontbrekende tags ná consent: ${b.ontbrekende_tags_na_consent ? (b.ontbrekende_tags_na_consent.length ? b.ontbrekende_tags_na_consent.map((t) => t.tag).join(', ') : 'geen') : 'niet gemeten (geen meting 2)'}`);
